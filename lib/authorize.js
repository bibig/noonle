...
function getAdminPasswordCookieName(req, id) {
	return load.belongToRoot(id) ? load.rootId() : id;
}

function getAdminPasswordCookieValue (req, id) {
	return req.cookies['w_' + getAdminPasswordCookieName(req, id)];
}

// weak check, only check related cookie wether exist
function isAdmin (req) {
	var id = load.fetch(req, 'node', 'id');
	return getAdminPasswordCookieValue(req, id) ? true : false;
}
...
function submitForAdmin (req, res, next) {
	...
	if (password  == srcPassword) {
		...
		saveAdminCookie(res, getAdminPasswordCookieName(req, node.id), password);
...
function saveAdminCookie (res, id, pass) {
	res.cookie('w_' + id, encrypt(pass), cookieOptions); 
}
